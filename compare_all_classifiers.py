...
'''
def get_snorkel_labels(lstm_y, mlp_y, sub_y, path_y, true_y, indexes_annotated):
    snorkel_y = np.empty(len(lstm_y), dtype = np.int)
    snorkel_y.fill(-1);
...
def get_supervised_aggregation_labels(lstm_y, mlp_y, sub_y, path_y, true_y, indexes_annotated):
    sup_y = np.empty(len(lstm_y), dtype=np.int)
    sup_y.fill(-1);
    ...
    for train_split, test_split in kf.split(indexes_annotated):
        ...
        test_feature_list = [lstm_annotated_test, mlp_annotated_test, sub_annotated_test, path_annotated_test]
        L_test = np.transpose(np.vstack(tuple(test_feature_list)))
        y_predicted = model.predict(L_test)
        result = classification_report(true_annotated_test, y_predicted, output_dict=True)
...
def get_results(y_true, y_predicted):
    ...
    return  str(r2(result['1']['precision'])) + "\t" \
            +str(r2(result['1']['recall']))         + "\t" \
            +str(r2(result['1']['f1-score']))         + "\t" \
            + str(r2(result['accuracy'])) + "\t" \
            +str(r2(result['0']['precision'])) + "\t" \
            +str(r2(result['0']['recall']))         + "\t" \
            +str(r2(result['0']['f1-score']))
...
with open(logfile, "w") as log:
    for index, x in enumerate(tqdm(x_test_fil)):
        if index not in indexes_annotated:
            continue
        e = int(x[0])
        r = int(x[1])
        a = int(x[2])
