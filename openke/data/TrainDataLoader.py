...
class TrainDataLoader(object):
    ...
    def sampling_lp(self):
        res = []
        self.lib.getHeadBatch(self.batch_h_addr, self.batch_t_addr, self.batch_r_addr)
        res.append({
            "batch_h": self.batch_h.copy(),
            "batch_t": self.batch_t[:1].copy(),
            "batch_r": self.batch_r[:1].copy(),
            "mode": "head_batch"
        })
        self.lib.getTailBatch(self.batch_h_addr, self.batch_t_addr, self.batch_r_addr)
        res.append({
            "batch_h": self.batch_h[:1],
            "batch_t": self.batch_t,
            "batch_r": self.batch_r[:1],
            "mode": "tail_batch"
        })
        return res
    ...
    def __iter__(self):
        ...
        elif self.sampling_mode == "link":
            self.lib.initTest()
            return TrainDataSampler(self.trainTotal, self.sampling_lp)
        else:
